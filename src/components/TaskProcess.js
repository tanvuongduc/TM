...
const TaskProcess = ({
  ...
  const handleRemoveTask = async () => {
    if (_id) {
      await taskListApi.deleteTask(_id);
      onResetPackage();
      onReloadpage();
    } else return;
  };
  ...
  const handleAddTask = async () => {
    await taskListApi.addTask(userID, taskName, priority, status);
    onReloadpage();
  };

  const handleModifyTask = async () => {
    await taskListApi.modifyTask(_id, taskName, priority, status);
    onResetPackage();
    onReloadpage();
  };
  ...
  const displayRemoveButton = () =>
    removeBtn || taskName ? (
      <Button className="removeBtn" onClick={handleRemoveTask}>
        Remove
      </Button>
    ) : (
      <Button disabled className="removeBtn faded">
        Remove
      </Button>
    );
  ...
  return (
    <Col xl="4" lg="4" md="4" className="taskProcess">
      <div className="editArea">
        <FormGroup className="editArea-formGroup">
          ...
          <Input
            ...
            value={taskName}
            onChange={(e) => editTaskProcess(e.target.value)}
          />
        </FormGroup>
        <FormGroup className="editArea-formGroup">
      ...
      <div className="actionArea">
        {displayRemoveButton()}
        <Button className="cancelBtn" onClick={handleCancelProgress}>
          Cancel
        </Button>
